...
function init_(){
	...
	function sense_OpenDiv(show_id,_Dw,_Dh,_Desc0) {
		...
		OpenNow(show_id,_Dw);
		document.getElementById('sense_mbDIV').style.display = '';
	}
	...
	function CloseNow() {
		...
		if(Cw>0||Ch>0){
			...
			document.getElementById('sense_mbDIV').style.display = 'none';
			//init_(show_id).innerHTML='';
		}
	}
 
	function sense_ttt(show_id){
	...
	$(window).ready(function() {
		...
		var m_h		=	d_h<b_h?b_h:d_h;
          ...
          document.getElementById('sense_mbDIV').style.height = m_h + "px";
...
function open_div(src,div_id){
	div_id		=	arguments[2]?arguments[2]:'Loading';

	$("#sense_iframe_id").attr('src',src);
	sense_OpenDiv(div_id,620,30,'');
}

function sense_open_div_extends(src,div_id,w,h){
	div_id		=	arguments[2]?arguments[2]:'Loading';
	w			=	arguments[3]?arguments[3]:'620';
	h			=	arguments[4]?arguments[4]:'30';

	$("#"+div_id+"_iframe_id").attr('src',src);
	sense_OpenDiv(div_id,w,h,'');
}